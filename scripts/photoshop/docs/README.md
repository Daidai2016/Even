# Photoshop Automation Guide


版本：V2.2



# 一、文档定位


本文档用于说明：

Photoshop 在 Codex Design 项目中的自动化生产定位。


本文档是能力与工作流指南，不定义强制开发规则。


Photoshop主要作为：

位图图像处理与后期制作中心。


主要应用：

- 照片修饰
- 图像创作
- 图像调整
- 特效处理
- AI生成图像后处理
- 批量图像生产
- 印刷生产处理



项目通用开发规范：

参考：

CODING_RULES.md


Photoshop 局部强制规则：

参考：

scripts/photoshop/AGENTS.md


Adobe整体自动化体系：

参考：

scripts/common/docs/ADOBE_AUTOMATION_GUIDE.md



---


# 二、Photoshop自动化方向


Photoshop自动化主要服务于：

图像生产流程。


重点建设方向：


- 图像处理自动化
- 批量生产流程
- 图层管理自动化
- 文件处理流程
- AI辅助设计流程



---


# 三、图像处理自动化


Photoshop适用于：

对位图图像进行自动化处理。


应用：


- 图像调整
- 色彩处理
- 图像优化
- 特效制作
- 图像合成



自动化目标：


减少重复操作。

保持设计效果稳定。



---


# 四、照片修饰与图像创作


Photoshop作为：

图像创作和后期处理平台。


应用：


- 照片修饰
- 素材优化
- 图像合成
- 创意效果制作



自动化重点：


建立可重复的制作流程。



---


# 五、图层自动化


Photoshop自动化涉及：

PSD文件结构管理。


应用：


- 图层创建
- 图层整理
- 图层属性处理
- 模板化制作



目标：


保持文件结构清晰。

方便后续编辑和生产。



---


# 六、批量生产流程


Photoshop适用于：

大量图像文件处理。


典型应用：


- 批量调整
- 批量转换
- 批量保存
- 批量输出
- 批量处理生产文件



重点：


适用于重复性、高数量生产任务。



---


# 七、图像尺寸与分辨率处理


Photoshop处理图像时：

必须区分：


- 物理尺寸
- 像素尺寸
- 分辨率属性



其中：


PPI：

表示图像分辨率属性。



DPI：

表示制作输出要求。



修改图像尺寸时：

必须明确修改目标。


需要根据实际需求判断：


- 是否保持像素数量不变
- 是否改变物理尺寸
- 是否调整分辨率属性


未经确认：

不得改变图像规模。



---


# 八、AI辅助图像流程


Photoshop作为：

AI生成内容的后期处理平台。


典型流程：


AI生成素材

↓

Photoshop优化

↓

图像合成

↓

设计制作

↓

输出文件



应用：


- AIGC素材处理
- 图像优化
- 创意合成
- 后期调整



---


# 九、印刷生产应用


Photoshop参与：

印刷文件制作和后期处理。


主要关注：


- 图像质量
- 色彩环境
- 文件完整性
- 制作要求一致性



具体制作规范：

遵循项目实际生产要求。



---


# 十、与Illustrator协同流程


典型流程：


Illustrator：

矢量图形、文字、版式制作


↓

图像素材处理


↓

Photoshop：

图像调整、合成、效果处理


↓

完成生产文件



---


# 十一、Photoshop脚本组织结构


参考目录结构：


photoshop/


├── jsx/

基础 Photoshop 脚本


├── actions/

Photoshop动作流程


├── batch/

批量处理流程


├── layers/

图层自动化


├── image/

图像处理


├── export/

输出流程


├── templates/

模板相关流程


├── ai-workflow/

AI辅助流程


├── examples/

示例


└── docs/

文档



---


# 十二、建设重点


优先建设：


1. 批量图像处理流程

2. 自动文件处理

3. 图层管理自动化

4. AI素材后处理

5. 印刷生产辅助流程

## 当前生产入口

- `../jsx/Even_大画面输出助手_v19.jsx`：单文件大画面输出；文件名是当前入口标识，文件头中的 V17 为历史注释，不能据此判定已回退。
- `../jsx/Even_大画面批量输出助手_v28.jsx`：批量大画面输出。两者处理范围不同，不相互替代。

本次仅校正导航说明，未修改生产脚本，也未新增 Adobe 真机验证结论。

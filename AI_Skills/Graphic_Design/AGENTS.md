...
3. 仅在当前主要目标或已建立的任务上下文匹配时读取 `skills/<skill-name>/SKILL.md`；通用画幅、字体或品牌色要求本身不触发花卉 Skill。没有匹配 Skill 时直接使用适用规则。
...
- 新建或修改 Skill 及其依赖时，按第 5 节执行一次受影响范围的结构校验，并按根规则执行仓库验证；相同版本、检查项和范围的结果直接复用。仅使用 Skill 不触发结构校验。仅报告本次交付范围内未完成的视觉、字体、版权、印刷或视频检查。
